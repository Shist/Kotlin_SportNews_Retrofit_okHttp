...
### Смотри его код из видосика ###
##################################################
2) Внутри Activity нужно делать не ".add", а ".replace" и добавлять его в BackStack(), чтобы можно было по кнопке back вернуться назад на наш layout (Туда нужно передать новый фрагмент - NewsPageFragment)
...
5) Нам нужен отдельный котлин-класс NewsPageFragment, туда засунуть переменную pageText (данные для разных активити отдельно пишутся соответственно в разных классах, каждому классу своя xml-ка)
